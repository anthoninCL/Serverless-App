...
export type ScreenProps = NativeStackScreenProps<RootStackParamList, 'register'>;
...
const RegisterScreen = ({ navigation }: ScreenProps) => {
  ...
  const handleRegister = async (username: string, email: string, password: string) => {
    const isUserLogged = await register(username, email, password);

    if (isUserLogged === true || (typeof isUserLogged === 'object' && 'id' in isUserLogged)) {
      ...
      <View style={{
        ...
        <View style={{flex: 1}}>
          ...
          <ViewRow align={"center"}>
            <JText style={styles.navigation} labelKey={"auth.alreadyHaveAnAccount"}/>
            <TextButton onPress={handleAlreadyHaveAnAccount} labelKey={"auth.signin"} fontSizeName={"large"} isBold={true}/>
          </ViewRow>
        </View>
      </View>
    </AuthLayout>
