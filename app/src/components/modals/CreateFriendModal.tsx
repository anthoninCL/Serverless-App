    ...
    <Overlay isVisible={props.isVisible} onBackdropPress={props.onBackDropPress} overlayStyle={{borderRadius: 10, maxWidth: 800}}>
      <ViewCol style={{paddingHorizontal: 20, paddingVertical: 20}}>
        <ViewRow align={'center'} justify={'space-between'} style={{width: '100%'}}>
          <Text style={{fontWeight: '900', fontSize: theme.fontSizes.huge}}>Add a friend</Text>
          <ClickableIcon type={"IonIcons"} name={"close"} onPress={props.onBackDropPress} colorName={"lightHigh"} sizeName={'huge'}/>
        </ViewRow>
        <Text style={{marginTop: 20, marginBottom: 30, color: theme.colors.lightHigh, fontSize: 18, paddingRight: 20}}>You can add friends to talk with them in private messages. Don't worry, you can remove them if they're annoying.</Text>
        <Text>LUCAS FDP METS TON USER ICI</Text>
        <ViewRow align={'center'} style={{marginTop: 20}}>
          <Text style={{color: theme.colors.lightHigh, fontSize: 15, paddingRight: 80}}>Before adding this friend, make sure you're not doing any mistake by adding wrong people to your friends.</Text>
          <ClassicButton onPress={() => {
          }} labelKey={"common.create"} type={'classic'} enabled={buttonEnabled} />
        </ViewRow>
      </ViewCol>
    </Overlay>
  );
